...
print("DS4Bolivia - Predicting Poverty from Satellite Embeddings")
print("="*80)
...
overpred = results_df.nsmallest(5, 'residual')[['mun', 'dep', 'index_sdg1', 'predicted_sdg1', 'residual']]
for idx, row in overpred.iterrows():
    print(f"    {row['mun']:30s} ({row['dep']:15s}): Actual={row['index_sdg1']:.2f}, Pred={row['predicted_sdg1']:.2f}, Diff={row['residual']:.2f}")

print(f"\n  Top 5 Underpredictions (Model predicts LOWER than actual):")
underpred = results_df.nlargest(5, 'residual')[['mun', 'dep', 'index_sdg1', 'predicted_sdg1', 'residual']]
...
print("  4. Investigate municipalities with large errors for local contextual factors")
